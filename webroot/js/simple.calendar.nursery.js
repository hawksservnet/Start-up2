	calendar = {
			...
			selectDate:function(year,month,day) {
				...
				if(ths.opt['onDateSelect']) ths.opt['onDateSelect'].apply(ths, [year,month,day]); // Custom handler if the user wants it that way.
					...
					document.getElementById(ths.opt["input"]).value = _date; // Date format is :HARDCODE:
                    var func=$('#'+ths.opt["input"]).attr('data-func')
                    var _id=$('#'+ths.opt["input"]).attr('data-id');
                    if (func=='change_date'){
                        change_date(_id,_date);
                    }
                    else if (func == 'change_cancel_limit'){
                        change_cancel_limit(_id,_date);
                    }

					ths.hideCalendar();
				}
			},
			/// Called when the user clicks on clear button.
			removeDate:function() {
				var ths = _calendar_active_instance;
				document.getElementById(ths.opt["input"]).value = ''; // Date format is :HARDCODE:
				//ths.hideCalendar();
			},
			...
			makeCalendar:function(year, month, day) {
				...
				this.wrt("</table>");
				this.wrt("<input type='button' value='閉じる' class='calendar-cancel' onclick='calendar.hideCalendar();' />");
				//this.wrt("<input type='button' value='削除' class='calendar-remove' onclick='calendar.removeDate();' />");
			...
			showCalendar: function() {
				...
				var position=$('#'+this.opt['input']).parent();

				var xy= {
                    0: position.offset().left,
                    1:position.offset().top ,
                }
